...
const generateWb = (month: number, names: string[], config: Configs): Workbook => {
  ...
  const blocks = getBlocksForTheMonth(first, last.getDate(), rowResolver, [], getExcludedColumns(config.days), config.formulas, config.locale);
  names.forEach((name) => {
    const sheet = wb.addWorksheet(name);
    blocks.forEach((block) => writeBlock(sheet, block, config.formulas, config.startHeader, config.endHeader));
